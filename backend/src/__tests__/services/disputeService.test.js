jest.mock("../../repositories/disputeRepository", () => ({
  ...
  findByLicenseId: jest.fn(),
  updateDisputeStatus: jest.fn(),
  findOpenDisputes: jest.fn(),
  recordArbitratorVote: jest.fn(),
  findVotesByDisputeId: jest.fn(),
  upsert: jest.fn(),
  resolve: jest.fn(),
...
describe("disputeService unit tests", () => {
  ...
  describe("hashEvidence", () => {
    ...
    it("is stable across key ordering for object bundles", () => {
  ...
  describe("castArbitratorVote", () => {
    it("records FullRefund vote", async () => {
      ...
      disputeRepository.recordArbitratorVote.mockResolvedValue({
        id: 1,
        disputeId: 10,
